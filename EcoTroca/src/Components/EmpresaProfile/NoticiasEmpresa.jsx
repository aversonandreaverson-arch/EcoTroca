...
const FORM_VAZIO = { tipo_publicacao: 'noticia', titulo: '', descricao: '', provincia: '', imagem: '' };

export default function NoticiasEmpresa() {
  ...
  const carregar = async () => {
    try {
      ...
      setPublicacoes(feed.filter(p => p.tipo_publicacao === 'noticia'));
    } catch (err) {
      setErro(err.message);
    } finally {
      setCarregando(false);
    ...
    <div className="min-h-screen bg-green-100 pt-24 pb-12 px-6">
      ...
      <div className="flex justify-between items-start mb-6 flex-wrap gap-4">
        <div>
          <h1 className="text-3xl font-bold text-green-800 flex items-center gap-2">
            <Newspaper size={28} /> Notícias
          </h1>
          <p className="text-gray-500 mt-1">Notícias sobre reciclagem e ambiente publicadas na plataforma.</p>
        </div>
        <button
      ...
      {carregando ? (
        ...
        <div className="bg-white border border-green-100 rounded-2xl p-12 text-center shadow-sm">
          ...
          <p className="text-gray-500 font-medium">Nenhuma notícia publicada ainda.</p>
          <p className="text-gray-400 text-sm mt-1">Clica em "Nova Notícia" para começar.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {publicacoes.map(p => (
            <div key={p.id_publicacao} className="bg-white border border-cyan-100 rounded-2xl overflow-hidden shadow-sm">
              {p.imagem && (
              ...
              <div className="p-4">
                <div className="flex items-center justify-between mb-2">
                  <span className="bg-cyan-100 text-cyan-700 text-xs font-medium px-2 py-0.5 rounded-lg">
                    Notícia
                  </span>
                ...
                <div className="flex items-center justify-between pt-2 border-t border-gray-100">
                  ...
                  {(utilizador?.tipo === 'admin' || utilizador?.id === p.id_autor) && (
                    <button onClick={() => handleApagar(p.id_publicacao)}
        ...
        <div className="fixed inset-0 bg-black/60 flex items-end md:items-center justify-center z-50 px-0 md:px-4">
          <div className="bg-white rounded-t-3xl md:rounded-2xl p-6 w-full max-w-lg shadow-xl">
            <div className="flex justify-between items-center mb-5">
              <h3 className="text-green-800 font-bold text-lg">Nova Notícia</h3>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="text" value={formulario.titulo}
                  ...
                  placeholder="Ex: Angola recicla 30% mais em 2025"
              ...
              <div>
                ...
                <textarea value={formulario.descricao}
                  ...
                  placeholder="Escreve o conteúdo da notícia..." rows={5}
