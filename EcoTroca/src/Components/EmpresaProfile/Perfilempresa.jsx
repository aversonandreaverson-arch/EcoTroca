    ...
    <div className="min-h-screen bg-green-100 pt-24 p-6">
      ...
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-green-100 p-6">
          ...
          <div className="space-y-3 text-gray-600">
            ...
            <div className="flex items-center gap-3">
              ...
              <span>
                {[perfil?.endereco, perfil?.bairro, perfil?.municipio, perfil?.provincia].filter(Boolean).join(', ') || 'Localização não definida'}
              </span>
            </div>
            {perfil?.site && (
              <div className="flex items-center gap-3">
        ...
        <div className="grid grid-cols-2 gap-4">
          {ehProprioPerfilEmpresa && (
            <>
              <button onClick={() => navigate('/EditarEmpresa')} className="bg-green-600 hover:bg-green-700 text-white py-3 rounded-xl font-medium transition">
                Editar Perfil
              </button>
              <button onClick={() => navigate('/DashboardEmpresa')} className="bg-white hover:bg-gray-50 text-green-700 border border-green-200 py-3 rounded-xl font-medium transition">
                Dashboard
              </button>
            </>
          )}
          {utilizador?.tipo_usuario === 'comum' && id && (
            <button onClick={() => navigate(`/NovoResiduo?empresa=${id}`)} className="col-span-2 bg-green-600 hover:bg-green-700 text-white py-3 rounded-xl font-bold transition flex items-center justify-center gap-2">
