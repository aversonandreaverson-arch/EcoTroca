...
import {
  ...
  Banknote, Plus, Search, Trash2, X, HandshakeIcon
} from 'lucide-react';
...
export default function PaginaInicial() {
  ...
  const [interesseEnviado,  setInteresseEnviado]  = useState({}); // { id_publicacao: true }
  ...
  const handleEnviarInteresse = async () => {
    ...
    if (!valorProposto || vProposto <= 0) {
      setErroInteresse('Indica um valor proposto em Kz.');
      return;
    }
    if (vMin && vProposto < vMin) {
      setErroInteresse(`O valor mínimo para este resíduo é ${vMin} Kz/kg.`);
      return;
    }
    if (vMax && vProposto > vMax) {
      setErroInteresse(`O valor máximo para este resíduo é ${vMax} Kz/kg.`);
      return;
    }
    try {
      ...
      await criarNotificacao({
        ...
        mensagem: `${nomeEmpresa} quer comprar o teu resíduo "${tituloResiduo}" por ${parseFloat(valorProposto).toFixed(0)} Kz/kg.${mensagemInteresse ? ` Nota: ${mensagemInteresse}` : ''}`,
      });
              ...
              <div>
                ...
                {publicacaoAlvo.preco_min && publicacaoAlvo.preco_max && (
                  <p className="text-xs text-gray-400 mt-1">
                    Referência de mercado: {publicacaoAlvo.preco_min}–{publicacaoAlvo.preco_max} Kz/kg
                  </p>
                )}
              </div>

              <div>
                <label className="text-gray-600 text-sm block mb-1">Nota para o utilizador (opcional)</label>
                <textarea
...
function CartaoPublicacao({ publicacao: p, utilizador, tipoUtilizador, onApagar, onInteresse, interesseJaEnviado }) {
  ...
  // Empresas podem mostrar interesse em ofertas de resíduo de outros utilizadores
