...
const registar = async ({ nome, email, telefone, senha, tipo_usuario, provincia, municipio, bairro }) => {
  const [existe] = await pool.query('SELECT id_usuario FROM Usuario WHERE email = ? OR telefone = ?', [email, telefone]);
...
const login = async ({ email, senha }) => {
  const [rows] = await pool.query('SELECT * FROM Usuario WHERE email = ? AND ativo = TRUE', [email]);
